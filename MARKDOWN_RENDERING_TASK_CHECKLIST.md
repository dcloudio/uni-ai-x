...
| T05 | 侧滑菜单不抢占表格/代码横滑，不误触垂直滚动 | 已完成并验收 | 应用层 | `3dc1a7d`；Android 五个真实手势场景全部通过 |
...
- 设备系统同时拒绝 `adb input swipe` 和 Monkey 脚本注入触摸事件（缺少 `INJECT_EVENTS`，注入事件数为 0）；官方 `program.swipe` 也注明 Android 10 以上不生效，因此没有用自动注入冒充真机验收。
- 2026-07-30 用户在 Android 14 真机依次执行宽表格横滑、代码块横滑、正文垂直滚动、左缘短滑打开侧栏和向左滑动关闭侧栏，五项均明确确认通过。
- 完整设备环境、命令、步骤、逐项结果、截图及 SHA-256 见 [`test-results/android-drawer-gesture-acceptance-results.txt`](test-results/android-drawer-gesture-acceptance-results.txt)。
...
- 侧滑菜单误触：方向锁、左缘起手、速度吸附和横向 ScrollView 让权均已实现，Android 五个真实手势场景已验收通过。
...
| 日期 | 编号 | 更新内容 | 提交/报告 |
| --- | --- | --- | --- |
| 2026-07-31 | T01/T02 | 修复 iOS Worker 生成 Swift 的剩余类型错误；Xcode 26.3 云打包成功，基座 2.1.5 安装并同步到 iPhone；Android 6 页 clean compile 和八场景真机回归通过；iOS 运行时因设备锁屏未验收 | `09934d3`、`54ac333`；[`test-results/ios-worker-cloud-build-and-android-regression-results.txt`](test-results/ios-worker-cloud-build-and-android-regression-results.txt) |
