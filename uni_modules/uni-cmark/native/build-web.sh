...
WEB_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../utssdk/web" && pwd)
...
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/uni-cmark-web.XXXXXX")
...
"$UNI_CMARK_CMAKE_BIN" --build "$BUILD_DIR" --target cmarkhtml_web --parallel
cp "$BUILD_DIR/cmark-gfm-md2html.mjs" "$WEB_DIR/cmark-gfm-md2html.mjs"

echo "Built $WEB_DIR/cmark-gfm-md2html.mjs with cmark-gfm $CMARK_VERSION"
