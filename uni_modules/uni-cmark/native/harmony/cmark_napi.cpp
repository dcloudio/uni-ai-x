...
static napi_value Md2json(napi_env env, napi_callback_info info) {
  size_t argc = 1;
  napi_value args[1];
  napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
  if (argc != 1) {
    napi_throw_type_error(env, nullptr, "md2json expects one Markdown string");
    return nullptr;
  }
  ...
  char *json = uni_cmark_markdown_to_json(
      ...
      &json_length);
  if (json == nullptr) {
    napi_throw_error(env, nullptr, "cmark-gfm failed to convert Markdown to JSON");
    return nullptr;
  }

  napi_value result;
  napi_status status = napi_create_string_utf8(env, json, json_length, &result);
  uni_cmark_free_json(json);
  if (status != napi_ok) {
    napi_throw_error(env, nullptr, "Failed to create the Markdown JSON string");
    return nullptr;
  }
  return result;
}
...
static napi_value Init(napi_env env, napi_value exports) {
  napi_property_descriptor descriptors[] = {
      {"md2html", nullptr, Md2html, nullptr, nullptr, nullptr, napi_default,
       nullptr},
      {"md2json", nullptr, Md2json, nullptr, nullptr, nullptr, napi_default,
       nullptr},
  };
